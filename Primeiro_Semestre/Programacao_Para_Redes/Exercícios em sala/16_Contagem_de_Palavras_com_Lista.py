print("CONTANDO INCIDÊNCIA DE PALAVRAS\n")

T = """Procurar a nossa felicidade através da felicidade dos outros - aconselhava Olívia noutra carta sem data.
...
novo com ele. Era uma mulher nova.""".upper()

C_REMOVE = [".",",","?","-"]

#REMOVENDO CARACTERES NÃO ESSENCIAIS DA STRING
for i in C_REMOVE:
    T = T.replace(i,"")

#QUEBRANCO A STRING A PARTIR DOS ESPAÇOS ENTRE AS PALAVRAS
WL = T.split()
...
for i in WL:
    if i not in CONT_WL:
        ...
        CONT[CONT_WL.index(i)] += 1

PAR = []

for i in range(len(CONT_WL)):
    PAR.append([CONT_WL[i],CONT[i]])
        
PAR.sort(key=lambda x: -x[1])

for i in PAR:
    print("LETRA:",i[0],"| QTD:",i[1])
